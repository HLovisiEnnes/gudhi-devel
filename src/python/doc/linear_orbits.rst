...
Definitions
-----------
A Lie group :math:`G` corresponds to a group with a smooth manifold structure, compatible with the group's prodcut. Here, we will
...
A representation or linear action is a smooth group morphism :math:`\rho: G \to GL(R, n)`, where :math:`GL(R, n)` corresponds to the
group of :math:`n \times n` invertible real matrices.

Given a non-zero point :math:`x\in R^{n}`, the orbit of :math:`x` under :math:`\rho` is :math:`O_x=\{\rho(g)\cdot x| g \in G\}`.
Orbits of :math:`T^d` are dimension :math:`d` submanifolds of :math:`R^{n}`, whereas orbits of :math:`SO(3)`, and :math:`SU(2)`
