...
def find_closest_algebra(
    ...
    if method in ["bottom_lie_pca", "abelian"]:
        ...
        vecs = [vecs[:, i].reshape((ambient_dim, ambient_dim)) for i in range(group_dim)]
    ...
    if method in ["bottom_lie_pca", "full_lie_pca"]:
        ...
        canonical_algebra = get_canonical_pushforward_algebra(group=group, rep_type=optimal_rep)
        optimal_algebra = [optimal_change_of_basis @ mat @ optimal_change_of_basis.T for mat in canonical_algebra]
...
def optimization_bottom_lie_pca(
    ...
    """
    # Defines basis
    canonical_algebra = get_canonical_pushforward_algebra(group=group, rep_type=rep_type)
...
def optimization_full_lie_pca(
    ...
    def _cost_function(orth):
        ...
        algebra = [orth @ mat @ orth.T for mat in canonical_algebra]
        # Computes cost
        differences = [lie_pca.dot(algebra[i].flatten()) for i in range(len(canonical_algebra))]
...
def optimization_abelian(lie_pca_algebra: list, reps_to_test: list) -> tuple[tuple, list, dict]:
    """
    ...
    if group_dim == 1:
        opt_weights, opt_change_of_basis = normal_form_skew_sym_matrix(lie_pca_algebra[0])
        opt_weights = (opt_weights,)
    else:
        opt_weights, opt_change_of_basis = normal_form_skew_symmetric_matrices(lie_pca_algebra)
    ...
    for weights in reps_to_test:
        if group_dim == 1:
            ...
            costs[weights] = np.linalg.norm(np.array(optimal_invariant) - np.array(invariant))
        else:
            ...
            for perm in itertools.permutations(range(m)):
                for sign in itertools.product([-1, 1], repeat=m):
                    ...
                    costs[permuted] = np.linalg.norm(np.array(optimal_invariant) - np.array(invariant))

    # Deduces optimal representation
    optimal_rep = min(costs, key=costs.get)

    # Defines optimal_algebra
    optimal_algebra = get_canonical_pushforward_algebra(group="torus", rep_type=optimal_rep)
    optimal_algebra = [opt_change_of_basis @ mat @ opt_change_of_basis.T for mat in optimal_algebra]
...
def normal_form_skew_sym_matrix(mat: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    change_of_basis = change_of_basis[:, np.ravel(np.column_stack((2 * index, 2 * index + 1)))]
...
def normal_form_skew_symmetric_matrices(
    ...
    weights = tuple(
        [tuple([matrices_normal_form[i][2 * j + 0, 2 * j + 1] for j in range(m)]) for i in range(len(matrices))]
    )
